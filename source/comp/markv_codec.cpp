...
class MarkvModel {
 public:
  ...
  size_t id_index_chunk_length() const { return 5; }
 ...
 protected:
  ...
  explicit MarkvCodecBase(spv_const_context context,
      ...
      : validator_options_(validator_options),
        ...
        opcode_huffman_codec_(GetOpcodeHist()),
  ...
  uint32_t GetVectorComponentType(uint32_t type_id) const {
    const auto it = vstate_.all_definitions().find(type_id);
  ...
  // was defined.
  uint64_t GetMtfForCurOperand(bool* can_forward_declare);
  ...
  const HuffmanCodec<uint32_t>* GetMtfHuffmanCodec(uint64_t handle) {
  ...
  const HuffmanCodec<uint64_t>* GetNonIdWordHuffmanCodec(
      uint32_t opcode, uint32_t index) {
    const auto it = non_id_word_huffman_codecs_.find(
        std::pair<uint32_t, uint32_t>(opcode, index));
  ...
  std::unordered_set<uint32_t> result_ids_registered_for_mtf_;
  std::map<uint64_t, std::unique_ptr<HuffmanCodec<uint32_t>>>
      mtf_huffman_codecs_;
  HuffmanCodec<uint64_t> opcode_huffman_codec_;
  ...
  std::list<uint32_t> cur_function_parameter_types_;
  ...
  std::map<uint32_t, std::vector<uint32_t>> descriptor_to_ids_;
};
...
class MarkvEncoder : public MarkvCodecBase {
 ...
 private:
  ...
  // Writes mtf rank to bit stream. |mtf_handle| is used to determine the codec
  // scheme. |fallback_mtf_handle| is used if no codec defined for |mtf_handle|.
  // Returns false iff encoding fails.
  bool EncodeMtfRankHuffman(uint32_t rank, uint64_t mtf_handle,
                            uint64_t fallback_mtf_handle);
...
class MarkvDecoder : public MarkvCodecBase {
 ...
 private:
  ...
  // Reads mtf rank from bit stream. |mtf_handle| is used to determine the codec
  // scheme. |fallback_mtf_handle| is used if no codec defined for |mtf_handle|.
  // Returns false iff decoding fails.
  bool DecodeMtfRankHuffman(uint64_t mtf_handle, uint32_t fallback_mtf_handle,
                            uint32_t* rank);
...
void MarkvCodecBase::ProcessCurInstruction() {
  ...
  if (inst_.result_id) {
    ...
    if (opcode == SpvOpFunction) {
      ...
      for (uint32_t i = 3; i < def_inst->words().size(); ++i) {
        cur_function_parameter_types_.push_back(def_inst->word(i));
      }
    }
  }
  ...
  if (opcode == SpvOpFunctionEnd) {
    ...
    assert(cur_function_parameter_types_.empty());
  }
  ...
  result_ids_registered_for_mtf_.insert(inst_.result_id);
  ...
  const uint32_t descriptor = id_descriptors_.IssueNewDescriptor(inst_);
  descriptor_to_ids_[descriptor].push_back(inst_.result_id);

  multi_mtf_.Insert(GetMtfIdDescriptor(descriptor), inst_.result_id);
}

uint64_t MarkvCodecBase::GetMtfForCurOperand(bool* can_forward_declare) {
...
void MarkvEncoder::EncodeOperandWord(uint32_t word) {
  auto* codec = GetNonIdWordHuffmanCodec(inst_.opcode, operand_index_);
  ...
  const size_t chunk_length =
      GetOperandVariableWidthChunkLength(operand_.type);
  if (chunk_length) {
...
bool MarkvDecoder::DecodeOperandWord(uint32_t* word) {
  const std::function<bool(bool*)> read_bit =
      std::bind(&MarkvDecoder::ReadBit, this, std::placeholders::_1);
  ...
  if (codec) {
    ...
    const bool huffman_result =
        codec->DecodeFromStream(read_bit, &decoded_value);
...
bool MarkvEncoder::EncodeOpcodeAndNumOperands(
  ...
  auto* codec = GetOpcodeAndNumOperandsMarkovHuffmanCodec(GetPrevOpcode());
  if (codec) {
    if (codec->Encode(word, &bits, &num_bits)) {
      // The opcode was successfully encoded into bits/num_bits.
      writer_.WriteBits(bits, num_bits);
      return true;
    } else {
      // The opcode is not in the Huffman table. Write kMarkvNoneOfTheAbove
      // and then encode the opcode as numeric value.
  ...
  if (opcode_and_num_operands_huffman_codec_.Encode(word, &bits,
                                                    &num_bits)) {
    // The opcode was successfully encoded into bits/num_bits.
    writer_.WriteBits(bits, num_bits);
    return true;
    ...
    // The opcode is not in the Huffman table. Write kMarkvNoneOfTheAbove
    // and then encode the opcode as numeric value.
    bool result = opcode_and_num_operands_huffman_codec_.Encode(
        kMarkvNoneOfTheAbove, &bits, &num_bits);
    (void)result;
    assert(result);
    writer_.WriteBits(bits, num_bits);
    return false;
...
bool MarkvDecoder::DecodeOpcodeAndNumberOfOperands(uint32_t* opcode,
  ...
  uint64_t decoded_value = 0;
  const bool huffman_result =
      opcode_and_num_operands_huffman_codec_.DecodeFromStream(read_bit,
                                                              &decoded_value);
  assert(huffman_result);
  if (!huffman_result)
    return false;
...
bool MarkvEncoder::EncodeMtfRankHuffman(uint32_t rank, uint64_t mtf_handle,
                                        uint64_t fallback_mtf_handle) {
  const auto* codec = GetMtfHuffmanCodec(mtf_handle);
  if (!codec) {
    assert(fallback_mtf_handle != kMtfNone);
    codec = GetMtfHuffmanCodec(fallback_mtf_handle);
  }
...
bool MarkvDecoder::DecodeMtfRankHuffman(
    uint64_t mtf_handle, uint32_t fallback_mtf_handle, uint32_t* rank) {
  const auto* codec = GetMtfHuffmanCodec(mtf_handle);
  if (!codec) {
    assert(fallback_mtf_handle != kMtfNone);
    codec = GetMtfHuffmanCodec(fallback_mtf_handle);
...
bool MarkvEncoder::EncodeIdWithDescriptor(uint32_t id) {
  auto* codec = GetIdDescriptorHuffmanCodec(inst_.opcode, operand_index_);
  if (!codec)
    return false;
...
bool MarkvEncoder::EncodeRefId(uint32_t id) {
  ...
  uint64_t mtf = GetMtfForCurOperand(&can_forward_declare);
  if (mtf != kMtfNone && !can_forward_declare) {
    assert(result_ids_registered_for_mtf_.count(id));
...
bool MarkvEncoder::EncodeTypeId() {
  if (inst_.opcode == SpvOpFunctionParameter) {
    assert(!cur_function_parameter_types_.empty());
    assert(inst_.type_id == cur_function_parameter_types_.front());
    cur_function_parameter_types_.pop_front();
    return true;
  }
...
bool MarkvDecoder::DecodeTypeId() {
  if (inst_.opcode == SpvOpFunctionParameter) {
    assert(!cur_function_parameter_types_.empty());
    inst_.type_id = cur_function_parameter_types_.front();
    cur_function_parameter_types_.pop_front();
    return true;
  }
...
bool MarkvEncoder::EncodeResultId() {
  ...
  if (num_still_forward_declared) {
    ...
    if (multi_mtf_.RankFromValue(kMtfForwardDeclared,
      ...
      // This is a definition of a non-forward declared id. Write rank 0.
      // TODO(atgoo@google.com) Consider using a single-bit Huffman code for
      // this case.
      rank = 0;
      writer_.WriteBits(0, 1);
    }
...
spv_result_t MarkvEncoder::EncodeInstruction(
  ...
  // Write operands.
  for (operand_index_ = 0; operand_index_ < operands.size();
       ++operand_index_) {
    ...
    switch (operand_.type) {
      ...
      case SPV_OPERAND_TYPE_LITERAL_STRING: {
        const char* src =
            reinterpret_cast<const char*>(&instruction.words()[operand_.offset]);

        auto* codec = GetLiteralStringHuffmanCodec(opcode);
        if (codec) {
          uint64_t bits = 0;
          size_t num_bits = 0;
...
spv_result_t MarkvDecoder::DecodeOperand(
  ...
  switch (type) {
    ...
    case SPV_OPERAND_TYPE_OPTIONAL_LITERAL_STRING: {
      ...
      std::vector<char> str;
      auto* codec = GetLiteralStringHuffmanCodec(inst_.opcode);
      ...
      if (codec) {
        ...
        const bool huffman_result =
            codec->DecodeFromStream(read_bit, &decoded_string);
        assert(huffman_result);
        if (!huffman_result)
          return vstate_.diag(SPV_ERROR_INVALID_BINARY)
